...
export const exportGroupSnapshot = async (db: DatabaseSync, groupId: string): Promise<GroupSnapshotV1> => {
  const group = db
    .prepare('SELECT id, currency, closed FROM groups WHERE id = ?')
    .get(groupId) as { id: string; currency: string; closed: number } | undefined;
  ...
  const economicUnits = db
    .prepare(
      `SELECT id, group_id, owner_participant_id, name
       FROM economic_units WHERE group_id = ? ORDER BY id`
    )
    .all(groupId) as Array<{ id: string; group_id: string; owner_participant_id: string; name: string | null }>;
...
export const importGroupSnapshot = async (
  ...
  await transactionRunner.runInTransaction(async () => {
    const existing = db.prepare('SELECT id FROM groups WHERE id = ?').get(groupId) as { id: string } | undefined;
    if (existing) {
      throw new Error(`Group already exists: ${groupId}`);
    }

    db.prepare('INSERT INTO groups (id, currency, closed) VALUES (?, ?, ?)').run(groupId, groupCurrency, group.closed ? 1 : 0);

    for (const economicUnit of [...economicUnitRecords].sort((left, right) => left.id.localeCompare(right.id))) {
      db.prepare('INSERT INTO economic_units (id, group_id, owner_participant_id, name) VALUES (?, ?, ?, ?)').run(
        economicUnit.id,
