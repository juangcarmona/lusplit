...
import { assertGroupOpen, assertIsoDate, assertNonEmpty, assertPositiveMinor, getRequiredGroup } from '../common';
...
export class EditExpenseUseCase {
  ...
  async execute(input: EditExpenseInput, _authContext?: AuthContext): Promise<ExpenseModel> {
    ...
    const payerExists = participants.some(
      (participant) => String(participant.id) === String(nextExpense.paidByParticipantId)
    );
    if (!payerExists) {
